...
| `GET /api/graph?address=&n=` | **Bubble map** — the token's wallet-relationship graph for spotting bundles: `{ nodes[], edges[], clusters[], stats }`. `nodes` = `{a, bal, pct, bought, sold, role ("bundle"\|"sniper"\|"holder"), cluster}`; `edges` = `{a, b, kind ("bundle"\|"transfer"), amt?, n?}` (bundle = bought in the same block; transfer = moved the token wallet-to-wallet); `clusters` = connected groups `{id, size, wallets[], bal, pct, hasBundle, flag}` (`flag`=bundle to be aware of). Render force-directed, size by `pct`, colour by `role`, ring flagged clusters. |
