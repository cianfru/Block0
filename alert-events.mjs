...
//   • smart-convergence — CONSENSUS: ≥2 proven-PnL wallets BOUGHT one token within an hour of each other, weighted by
//                         their realised record, and the strength crossed the bar (replaces the flat "≥2 hold it")
...
const DEF = {
  ...
  smartMin: 2,                   // consensus needs at least this many proven wallets INSIDE the buy window
  consensusMin: 2.0,             // …and a record-weighted strength ≥ this (two solid records, or three thin ones)
  consensusFreshH: 3,            // …and the cluster's latest buy within this many hours (never fire on an old cluster)
...
const snap = (t) => ({ sellers: t.flags?.insiderSellersNow || 0, smart: t.smart?.count || 0, cons: t.smart?.consensus?.strength || 0, clean: false });
...
export function detectEvents(prev, tokens, opts = {}) {
  ...
  for (const t of tokens || []) {
    ...
    const cs = t.smart?.consensus;
    if (cs && cs.n >= o.smartMin && cur.cons >= o.consensusMin && was.cons < o.consensusMin && (cs.freshH ?? 0) <= o.consensusFreshH) {
      const span = cs.spanMin < 1 ? "the same minute" : `${cs.spanMin} min`;
      fire("smart-convergence", t, { smart: cur.smart, consensus: cur.cons, n: cs.n, spanMin: cs.spanMin, tight10: cs.tight10, risk: t.risk ?? null },
        `${cs.n} proven wallets bought within ${span} of each other · consensus ${cur.cons.toFixed(1)} (record-weighted) · risk ${t.risk ?? "—"}/100`);
    }
