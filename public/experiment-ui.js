...
(() => {
  ...
  async function load() {
    ...
    try {
      ...
      status.textContent = (r.enabled === false ? "Collection disabled. " : "") + (r.error ? "Collection issue: " + r.error + ". " : "") + "Last cycle: " + when(r.updated) + ". Scope: Pons API-visible catalogs; coverage is partial. " + (coverage.dueRemaining ? coverage.dueRemaining + " observations awaiting budget." : "");
